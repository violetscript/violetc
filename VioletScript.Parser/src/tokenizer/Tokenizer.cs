...
public sealed class Tokenizer {
    ...
    private void RaiseUnexpected() {
        throw new ParseException(Script.CollectProblem(
            AtEof ? Problem.SyntaxError(0, GetCharacterSpan(), new ProblemVars { ["t"] = TToken.Eof })
                :   Problem.SyntaxError(1, GetCharacterSpan(), new ProblemVars {})
        ));
    }
    ...
    private void ScanIdentifier(string s, bool reserveKeyword = true) {
        ...
        if (reserveKeyword && ReservedWords.IsReservedWord(s)) {
            if (Token.RawLength != s.Count()) {
                throw new ParseException(Script.CollectProblem(Problem.SyntaxError(2, GetCharacterSpan())));
            }
    ...
    private void ScanNumericLiteral(bool startsWithDot) {
        ...
        if (startsWithDot) {
            ...
            var startsWithZero = Lookahead() == '\x00';
            ++Index;
            ch = Lookahead();
            if (startsWithZero) {
                // 0x... 0X...
    ...
    private string ScanOptUnicodeEscapeForIdentifier(bool atIdStart, bool mustBeValid = true) {
        ...
        if (mustBeValid) {
            if (atIdStart && !SourceCharacter.IsIdentifierStart(ch)) {
                throw new ParseException(Script.CollectProblem(Problem.SyntaxError(1, GetCharacterSpan())));
            }
    ...
    private void ScanStringLiteral() {
        ...
        if (lineBreakFound) {
            throw new ParseException(Script.CollectProblem(Problem.SyntaxError(3, Token.Span)));
        }
    }
