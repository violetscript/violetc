...
internal class ParserBackend {
    ...
    public Problem SyntaxError(int id, Span span, ProblemVars args = null) {
        return Script.CollectProblem(Problem.SyntaxError(id, span, args));
    }

    public Problem VerifyError(int id, Span span, ProblemVars args = null) {
        return Script.CollectProblem(Problem.VerifyError(id, span, args));
    }

    public Problem Warn(int id, Span span, ProblemVars args = null) {
    ...
    public void RaiseUnexpected() {
        throw new ParseException(SyntaxError(0, Token.Span));
    }
    ...
    private Ast.DestructuringPattern OptConvertExpressionIntoDestructuringPattern(Ast.Expression e) {
        if (e is Ast.Identifier e_asId) {
            ...
            var fields = new List<Ast.ObjectDestructuringPatternField>{};
            foreach (var field in e_asOi.Fields) {
                if (field is Ast.Spread field_asSpread) {
                    ...
                    fields.Add((Ast.ObjectDestructuringPatternField) FinishNode(new Ast.ObjectDestructuringPatternField(field_asOf.Key, subpattern), field.Span.Value));
                }
            }
            return (Ast.DestructuringPattern) FinishNode(new Ast.ObjectDestructuringPattern(fields, e_asOi.Type));
        }
        return null;
    }
    ...
    private (Ast.Statement node, bool semicolonInserted) ParseIncludeSTatement(Context context, Span startSpan) {
        ...
        if (Script.FilePath != null) {
            ...
            resolvedPath = resolvedPath + (resolvedPath.EndsWith(".violet") ? "" : ".violet");
            string innerSource = "";
    ...
    private void RestrictModifiers(Span span, Ast.AnnotatableDefinitionModifier actualModifiers, params Ast.AnnotatableDefinitionModifier[] modifiersToRestrict) {
        foreach (var m in modifiersToRestrict)
            if ((actualModifiers & m) != 0) {
                SyntaxError(18, span, new ProblemVars {["m"] = m});
            }
    }
    ...
    private (Ast.Statement node, bool semicolonInserted) ParseProxyDefinition(DefinitionAttributes attribs, Context context, Ast.Identifier id) {
        ...
        if (@operator == null) {
            VerifyError(23, id.Span.Value, new ProblemVars { ["p"] = id.Name });
    ...
    private (Ast.Statement node, bool semicolonInserted) ParseEnumDefinition(DefinitionAttributes attribs, Context context, Span startSpan) {
        ...
        Ast.TypeExpression numericType = Consume(TToken.Colon) ? ParseTypeExpression() : null;
    ...
    private (Ast.Statement node, bool semicolonInserted) ParseIfStatement(Context context) {
        ...
        NextToken();
        Expect(TToken.LParen);
        Ast.Node test = ((Ast.Node) ParseOptSimpleVariableDeclaration()) ?? ((Ast.Node) ParseExpression());
        Expect(TToken.RParen);
    ...
    private (Ast.Statement node, bool semicolonInserted) ParseBreakStatement(Context context) {
        ...
        if (label != null && !context.HasLabel(label)) {
            VerifyError(9, r.Span.Value, new ProblemVars { ["label"] = label });
